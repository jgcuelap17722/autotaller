    ...
    switch (tipoCaso) {
      case 0: // A entra NaN osea no activo el Switch / 0
        ...
        if (id_cliente != '') { // recibimos ID_CLIENTE BUSQUEDA
          ...
          const {
            placa, idMarca, idModelo, idGeneracion, color, id_cliente
          } = data;
          ...
          const {
            placa, idMarca, idModelo, idGeneracion, color
          } = data;
          const tipo_cliente = estado_tipo_cliente[0];
          const {
            nombre_cliente, telefono, email, dni, ruc, razon_social, direccion
          } = data;
      ...
      case 2:
        ...
        if (id_cliente != '') { // recibimos ID_CLIENTE
          ...
          const {
            placa, idMarca, modelo, anio, color, id_cliente
          } = data;
          ...
          const {
            placa, idMarca, modelo, anio, color
          } = data;
          const tipo_cliente = estado_tipo_cliente[0];
          const {
            nombre_cliente, telefono, email, dni, ruc, razon_social, direccion
      ...
      case 3:
        ...
        if (id_cliente != '') { // recibimos ID_CLIENTE
          console.log('Usando id_tipo_cliente:', estado_tipo_cliente[0]);
          const {
            placa, idMarca, idModelo, anio, color, id_cliente
          } = data;
          if (estado_tipo_cliente[1] != 0) {
            const tipo_cliente = estado_tipo_cliente[0];
          ...
          const {
            placa, idMarca, idModelo, anio, color
          } = data;
          const tipo_cliente = estado_tipo_cliente[0];
          const {
            nombre_cliente, telefono, email, dni, ruc, razon_social, direccion
...
router.post('/nuevo-cliente', async (req, res) => {
  ...
  if (existenciaPlaca != 0) {
    ...
    const { nuevaPlaca } = req.body;
    const id_marca = {
      InfoUser, id, marca, nuevaPlaca, nombre_dni, pId_cliente, pnombre_cliente, Tipos_Cliente
    };
...
router.post('/registro-completo', isLoggedIn, async (req, res, next) => {
  const InfoUser      = await helpers.InfoUser(req.user.id_usuario); // Info Usuario Logueado
  const {
    tipo_usuario, nombre, apellido_paterno, apellido_materno, telefono, dni, direccion, sexo, edad, email
  } = req.body;
...
router.get('/historial', isLoggedIn, async (req, res) => {
  ...
  const data = {
    historial, Tiempo_Inicio, Tiempo_Inicio_corto, InfoUser
  };
...
router.get('/detalle-seguimiento', isLoggedIn, async (req, res) => {
  ...
  const InfoUser = await helpers.InfoUser(req.user.id_usuario);
  const {
    idSeguimiento, id_cliente, id_vehiculo
  } = req.query;
  ...
  const Info_Cliente = {
    nombre_cliente, telefono, dni, ruc, tipo_cliente
