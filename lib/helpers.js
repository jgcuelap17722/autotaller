...
helpers.timeago_int = (pDate) => {
  ...
  pDate.setMinutes(pDate.getMinutes())
  pDate.setSeconds(pDate.getSeconds())
  return helpers.timeago(pDate)
}

helpers.new_Date = (pDate) => {

  // I (fecha string formato ISO ,Nombre timezone)
  Convercion_ISO_String = (pStr_pfecha,TimeZOne='UTC') => {
    var date = new Date(pStr_pfecha).toLocaleString('en-US',{
      hour12: false,
      timeZone: TimeZOne
    }).split(" ");
  ...
  let str_ISO = new Date(pDate).toISOString();

  let date_dddd = Convercion_ISO_String(str_ISO,'America/Lima')
  console.log('New Date es',date_dddd);

  let date_2 = pDate.setHours((pDate.getHours()+5))

  console.log('New Date_2 es',Convercion_ISO_String(date_2));
  
  let date = new Date(date_dddd)
...
helpers.formatDateTime = (date) => {
  ...
  // I (fecha string formato ISO ,Nombre timezone)
  Convercion_ISO_String = (pStr_pfecha) => {
    var date = new Date(pStr_pfecha).toLocaleString('en-US',{
      hour12: false
    }).split(" ");
 ...
 */    
    return Convercion_ISO_String(date_str)
  }else{
    return '';
  }
}
