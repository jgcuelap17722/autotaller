...
helpers.timeago_int = async (pDate) => {
  ...
  let fecha_extrangera = await moment(pDate).tz('Europe/Lisbon').format("YYYY-MM-DD HH:mm:ss")
  let mi_fecha = new Date();
  console.log('Fecha extrangera Europe/Lisbon',helpers.formatDateTime(fecha_extrangera));
  console.log('Fecha extrangera de Servidor',mi_fecha);

  let NewTimeago = await new Date(fecha_extrangera);
  console.log('Convercion de fechastring a date',helpers.formatDateTime(NewTimeago));

  const salida = await helpers.timeago(NewTimeago);
