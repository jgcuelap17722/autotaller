...
helpers.GenerarUsername = async (pEmail) => {
  try {
    ...
    const rows = await Consulta(consulta);
...
helpers.InfoUser = async (idUser) => {
  const consulta_info_User = await Consulta('SELECT id_usuario,nombre,apellido_paterno,sexo,id_tipo_usuario FROM v_tusuario_tpersona WHERE id_usuario = '+idUser+'');
...
helpers.timeago_int = (pDate) => {
  ...
  let Newdate = new Date(moment(pDate).tz('Europe/Lisbon').format("YYYY-MM-DD HH:mm:ss"));
      Newdate = helpers.timeago(Newdate);
  return Newdate
}
