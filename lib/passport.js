...
passport.use('local.signin', new LocalStrategy({
  ...
  const rows = await Consulta('SELECT * FROM tusuario WHERE username = ?', [username]);
  if (rows.length > 0) {
    const user = rows[0];
...
passport.use('local.signup', new LocalStrategy({
  ...
  const result = await Consulta('INSERT INTO tusuario SET ? ', [newUser]);
  console.log(result);
...
passport.deserializeUser(async (id_persona, done) => {
  const rows = await Consulta('SELECT * FROM tusuario WHERE id_usuario = ?', [id_persona]);
  done(null, rows[0]);
});